...
static char *ch_ltoa(char *p, unsigned long long num, unsigned radix) {
 ...
 *          The general parameters format is: %[-][width|*][.precision|*][l|L|ll|LL]p.
...
int chvprintf(BaseSequentialStream *chp, const char *fmt, va_list ap) {
  ...
  int n = 0;
  bool is_long, is_long_long, left_align;
  long long l;
#if CHPRINTF_USE_FLOAT
  float f;
...
unsigned_common:
      if (is_long)
        ...
        l = va_arg(ap, long long unsigned);
      else

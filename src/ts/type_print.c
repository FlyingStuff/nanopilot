...
bool ts_print_base_type(void (*print_fn)(void *, const char *, ...),
                            void *arg,
                            int type,
                            const void *p)
{
    ...
    if (entry->is_array || entry->is_dynamic_array) {
        ...
        for (index = 0; index < count; index++) {
            ...
            if (entry->is_base_type) {
                ...
                if (!ts_print_base_type(print_fn, arg, entry->base_type, entry_data)) {
                    return false;
                }
            } else {
                print_fn(arg, "\n");
                ts_print_type_indent(print_fn, arg, entry->type, entry_data, indent+1);
            }
        }
        if (!entry->is_base_type) {
            print_indentataion(print_fn, arg, indent);
        ...
        if (entry->is_base_type) {
            print_fn(arg, "%s: ", entry->name);
            if (!ts_print_base_type(print_fn, arg, entry->base_type, object)) {
                return false;
            }
            print_fn(arg, "\n");
        } else {
            print_fn(arg, "%s:\n", entry->name);
            ts_print_type_indent(print_fn, arg, entry->type, object, indent+1);
        }
    }
    return true;
}
                              ...
                              unsigned int indent)
{
    ...
    for (i = 0; i < type->nb_elements; i++) {
        if (!ts_print_entry(print_fn, arg, &type->elements[i], object, indent)) {
            return false;
        }
    }
    return true;
...
bool ts_print_type(void (*print_fn)(void *, const char *, ...),
                       ...
                       const void *object)
{
    return ts_print_type_indent(print_fn, arg, type, object, 0);
}

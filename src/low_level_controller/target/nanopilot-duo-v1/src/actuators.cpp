...
void actuators_set_output(const std::array<float, NB_ACTUATORS> out)
{
    ...
    pwm_banks[3].set_channel_pos_us(0, actuators[8].get_pulse_width(out[8]));
    pwm_banks[3].set_channel_pos_us(1, actuators[9].get_pulse_width(out[9]));

    pwm_banks[4].set_channel_pos_us(0, actuators[10].get_pulse_width(out[10]));

    pwm_banks[5].set_channel_pos_us(0, actuators[11].get_pulse_width(out[11]));
}

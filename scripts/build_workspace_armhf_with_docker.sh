...
docker run --rm -v `pwd`:`pwd`:cached -w `pwd` -e CROSS_COMPILE=arm-linux-gnueabihf- ros2-cross-compile \
    ...
    colcon build --build-base build_armhf --install-base install_armhf ${COLCON_ARGS} --cmake-args -DCMAKE_TOOLCHAIN_FILE=`pwd`/scripts/build/armhf_toolchainfile.cmake -DCMAKE_SYSROOT=`pwd`/sysroot"
